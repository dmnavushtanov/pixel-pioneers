...
/**
 * Enemy entity with health bar, movement toward a target, and death handling.
 */
export class EnemyUnit extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, def: EnemyDefinition) {
    ...
    // HP bar background
    this.hpBarBg = new Phaser.GameObjects.Rectangle(
      scene, 0, -(def.size + 8), def.size * 2, 4, 0x333333
    );
    // HP bar fill
    this.hpBar = new Phaser.GameObjects.Rectangle(
      scene, 0, -(def.size + 8), def.size * 2, 4, 0x44cc44
    );

    this.add([this.body_gfx, this.hpBarBg, this.hpBar]);
